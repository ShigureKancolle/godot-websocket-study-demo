...
============================================================================
    ...
    GameServer._process_tick(dt)
        └─ EnemyMgr.update(dt, room)                    # 每帧驱动 AI
               └─ 内部:读 room.get_entity() 取位置 → 决策 → 调 room.apply_move/apply_attack
============================================================================
"""
...
class EnemyMgr:
    ...
    def __init__(self):
        ...
        self._ai_states: Dict[str, EnemyAIState] = {}

    # ------------------------------------------------------------------
    # 生命周期回调(由 GameRoom.create_enemy / remove_entity 调用)
    # ------------------------------------------------------------------

    def on_enemy_created(self, entity_id: str, entity_type: str) -> EnemyAIState:
        """
        ...
        """
        if entity_id in self._ai_states:
            # 重复创建通常是 bug,早暴露
            raise ValueError(f"敌人 AI 状态已存在: {entity_id}")
        state = EnemyAIState(entity_id=entity_id)
        # entity_type 暂未使用,预留:后续可按类型选 AI 策略
        # (如 enemy_slime 用近战追击、enemy_ranged 用风筝射击)
        self._ai_states[entity_id] = state
        return state

    def on_enemy_removed(self, entity_id: str) -> Optional[EnemyAIState]:
        """
        敌人移除回调:在 GameRoom.remove_entity 里调用,清理 AI 状态

        幂等:不存在时不报错(和 GameRoom.remove_entity 的容错策略一致)。
        """
        return self._ai_states.pop(entity_id, None)

    # ------------------------------------------------------------------
    # 查询
    # ------------------------------------------------------------------

    def get_ai_state(self, entity_id: str) -> Optional[EnemyAIState]:
        """取单个敌人的 AI 状态(只读意图)"""
        return self._ai_states.get(entity_id)

    def is_enemy(self, entity_id: str) -> bool:
        """快速判断某个 entity_id 是不是敌人(AI tick 时过滤用)"""
        return entity_id in self._ai_states

    def get_all_enemy_ids(self) -> List[str]:
        """所有敌人的 entity_id 列表(快照,迭代期间安全)"""
        return list(self._ai_states.keys())

    def get_enemy_count(self) -> int:
        return len(self._ai_states)
