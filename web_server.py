...
# 先导入 message_bus，它会在内部把 proto/generated 加入 sys.path
from message_bus import MessageBus, MessageContext
import game_pb2
...
class GameServer:
    def __init__(self, host: str = "0.0.0.0", port: int = 8765):
        ...
        # 玩家信息表：player_id -> 字典（不再是 protobuf 对象）
    ...
    async def handle_client(self, websocket: websockets.WebSocketServerProtocol):
        ...
        async with websocket:
            ...
            ctx = MessageContext(websocket=websocket, player_id=player_id, is_server=True)
    ...
    async def cleanup_player(self, player_id: str):
        ...
        if player_id in self.player_infos:
            player_name = self.player_infos[player_id].get("player_name", "未知")
            del self.player_infos[player_id]
...
async def on_player_join(data: dict, ctx: MessageContext):
    """
    处理玩家加入
    ...
    # 用服务器分配的 ID 覆盖客户端传来的
    player_info["player_id"] = ctx.player_id

    # 保存玩家信息（直接存字典）
    server.player_infos[ctx.player_id] = player_info

    logger.info(f"玩家 {player_info.get('player_name', '未知')} (ID: {ctx.player_id}) 加入游戏")

    # 广播玩家加入消息给其他玩家
    await server.broadcast("PlayerJoin", {"player_info": player_info}, exclude_player=ctx.player_id)

    # 发送当前游戏状态给新玩家
    players_list = list(server.player_infos.values())
...
async def on_player_move(data: dict, ctx: MessageContext):
    """处理玩家移动"""
    if ctx.player_id not in server.player_infos:
        return

    # 更新玩家位置（直接操作字典）
    server.player_infos[ctx.player_id]["x"] = data.get("x", 0)
    server.player_infos[ctx.player_id]["y"] = data.get("y", 0)

    logger.debug(f"玩家 {ctx.player_id} 移动到 ({data.get('x')}, {data.get('y')})")
...
async def on_chat_message(data: dict, ctx: MessageContext):
    """处理聊天消息"""
    if ctx.player_id not in server.player_infos:
        return

    player_name = server.player_infos[ctx.player_id].get("player_name", "未知")
...
async def on_heartbeat(data: dict, ctx: MessageContext):
    """处理心跳消息"""
...
def start_console(server_instance, bus_instance, loop):
    ...
    namespace = {
        ...
        'player_infos': server_instance.player_infos,
    }
...
============================================================
 可用变量:
   ...
   player_infos  - 玩家信息表 {player_id: dict}
   game_pb2      - protobuf 模块
 可用函数:
 ...
 示例:
   len(players)                       # 查看在线人数
   list(player_infos.keys())          # 查看所有玩家ID
   player_infos                       # 查看所有玩家详情
